...
# to check the values for ucb function

# factor1 = (iteration_count**((d/2)+2))
# factor2 = (np.pi**2)
# factor3 = (1/(3*delta) )
# print (factor1, factor2, factor3)
# value = factor1*  factor2 * factor3
# print('value\n' ,value)
# beta3 = 2* np.log(value)
# print('beta3L: ', beta3)




    # def max_acq_func(self, gp_obj, Xs, ys):
    #
    ...
    #         fmax = 0
    #
    #         starting_points = np.random.uniform(self.lowerbound, self.upperbound, self.number_of_restarts)
    #         for starting_point in starting_points:
    #
    #             max_x = opt.minimize(lambda x: self.upper_confidence_bound_util(x, gp_obj), starting_point,
    #                                  method='L-BFGS-B',
    #                                  bounds=[(self.lowerbound, self.upperbound)])
    #
    #             mean, variance, f_prior, f_post = gp_obj.gaussian_predict(np.matrix(max_x['x']))
    #             std_dev = np.sqrt(variance)
    #             fvalue = self.ucb_acq_func(mean, std_dev)
    #
    #             if (fvalue > fmax):
    #                 print("New best Fval: ",fvalue," found at: ", max_x['x'])
    #                 x_max_value = max_x['x']
    #                 fmax = fvalue
    #
    #         print("Best is ", fmax, "at ", x_max_value)
    #
    #         with np.errstate(invalid='ignore'):
    #             mean, variance, f_prior, f_post = gp_obj.gaussian_predict(Xs)
    #             std_dev = np.sqrt(variance)
    #             acq_func_values = self.ucb_acq_func(mean, std_dev)
    #
    #     elif (self.acq_type == 'ei'):
    #
    #         mean_max = mean.max()
